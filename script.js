        ...
        async function setupNowarfyRemoteControl() {
            ...
            try {
                ...
                // Si no existe un reproductor activo —o su sesión venció— este primer dispositivo
                // debe tomar el iFrame real automáticamente. De lo contrario, Play enviaba comandos
                // a una sesión sin player y la interfaz quedaba aparentemente congelada.
        ...
        async function selectNowarfyPlaybackDevice(deviceId) {
            ...
            try {
                ...
                if (deviceId !== nowarfyRemoteDeviceId && wasLocalPlayer) {
                    // Cortar el audio aquí mismo evita que ambos dispositivos reproduzcan a la vez
                    // mientras llega el evento de Realtime al reproductor anterior.
                    stopAll();
                    isPlaying = false;
                    updateIcon();
                ...
                if (nowarfyRemoteIsPlayer) {
                    ...
                    // Al elegirse a sí mismo, este dispositivo no recibe un INSERT remoto.
                    // Ejecutamos el handoff localmente para cargar la cola, posición y volumen.
        ...
        async function sendNowarfyCommand(type, value = null) {
            ...
            // Volumen y seek viajan por Broadcast sin llenar la tabla con cada pixel del slider.
            // Si Broadcast no está listo o falla, se conserva el camino de Supabase como respaldo.
        ...
        const QUEUE_BATCH = 20;       // canciones que se suman en cada ronda de autogeneración
        // La siguiente tanda se genera recién al terminar la tanda actual; no se usa un umbral anticipado.
        let queue = [];                // lista completa de la cola actual
        let queueSeenKeys = new Set(); // para no repetir canciones dentro de la cola
        let queueIdCounter = 0;        // id incremental único por ítem de la cola (sobrevive a reordenes/borrados)
        let currentPlayingQid = null;  // _qid del ítem que está sonando ahora
        let currentSourceIdx = null;   // idx en currentList del resultado desde donde se eligió la canción actual (para el spinner)
        let queueRound = 0;            // 0 = ronda "mismo artista", 1+ = rondas "artistas similares"
        let queueFetching = false;     // evita pedidos de autogeneración duplicados
        let dragSrcQid = null;         // _qid del ítem que se está arrastrando en el panel de cola
        let searchQuery = "";
        ...
        // Evita insistir contra YouTube durante la misma sesión cuando informa que la cuota se agotó.
        ...
        // Preferencia actual: Nowarfy mantiene la continuidad en segundo plano.
        // Para escuchar otra aplicación, el usuario pausa Nowarfy manualmente.
        const NOWARFY_AUDIO_PRIORITY_MODE = true;
        const YT_PLAY_RETRY_THROTTLE_MS = 750;
        const YT_END_EARLY_TOLERANCE_SECONDS = 1.5;
        let channelFilter = null; // { id, title } cuando el usuario entra a ver el contenido de un canal
        ...
        initVolume();
        initNowarfyHistory();
        // Se difiere el inicio para que el perfil local y sus funciones ya estén inicializados.
        ...
        function normalizeSourceText(value) {
            ...
            // YouTube puede devolver entidades HTML dentro de texto plano; se decodifican antes de volver a escapar al renderizar.
        ...
        function hasValidArtwork(item) {
            ...
            // Las fuentes abiertas no siempre entregan thumbnail. En ese caso se usa
            // la portada local de Nowarfy, que también es válida para renderizar la tarjeta.
        ...
        function selectTasteTopic(topic) {
            ...
            // Al elegir un tema de interés se mantiene el filtro de canal si existe,
            // permitiendo buscar ese tema dentro del canal actual.
        ...
        function renderPersonalizedNote() {
            // La personalización sigue operando localmente; la portada ya no expone una nota descriptiva.
        }
        ...
        function handleSearchInput() {
            // Escribir no consulta nada: solo actualiza el estado visual del botón de limpiar.
        ...
        // Compatibilidad con accesos antiguos: nunca ejecuta una búsqueda por sí mismo.
        ...
        // Mezcla liviana (Fisher-Yates) para que el orden tampoco sea siempre idéntico
        ...
        // Convierte los items crudos de la YouTube Data API (video/playlist/channel) a nuestro formato interno
        ...
        async function loadChannelProfile(channelReference, channelTitle = '') {
            // Abrir un canal también es una navegación: minimizar el video antes de renderizar el catálogo.
            minimizeActiveVideoForNavigation();
            const loader = document.getElementById('loader');
            const container = document.getElementById('dynamicSections');
        ...
        // Obtiene resultados de YouTube una sola vez por petición. El proxy aplica caché compartida
        // y comunica los errores de cuota con un estado HTTP real, para no repetir solicitudes inútiles.
        function markYouTubeQuotaUnavailable() {
            // La página no vuelve a consultar YouTube durante una hora tras detectar el límite.
        ...
        // Formato "clásico" de pedido al backend (el que funcionaba antes): sin resourceTypes ni channelId.
        ...
        // No se fabrican playlists de emergencia: una lista solo aparece si YouTube o la reserva global/local la devolvieron realmente.
        ...
        async function performSmartSearch(query, options = {}) {
            ...
            // Al iniciar una búsqueda, liberar el área principal para mostrar los nuevos resultados.
            // El video continúa reproduciéndose y queda accesible desde el indicador flotante.
            minimizeActiveVideoForNavigation();
            searchQuery = query;
            ...
            try {
                ...
                // YouTube conserva la prioridad visual, pero Openverse se consulta siempre y se muestra en su propio rail.
        ...
        // Abre una ficha nativa con las listas públicas y los videos subidos del canal.
        ...
        async function fetchOpenverseTracks(query, limit = 20, options = {}) {
            ...
            // Openverse devuelve también `mp32` para streams de Jamendo y otros proveedores.
            // No aceptarlo hacía que la API respondiera correctamente pero la interfaz descartara todo.
        ...
        // Clasificación editorial: los videos no musicales siguen disponibles, pero quedan fuera de la radio y de los carruseles musicales.
        ...
        function isNonMusicalVideo(item) {
            ...
            // No confiar en contentGroup persistido: puede venir de una clasificación anterior y quedar obsoleto.
            ...
            // La categoría de YouTube por sí sola no alcanza: muchos videoclips están en Entertainment (24).
        ...
        function reviewVideoClassification() {
            // La revisión es deliberadamente local: no dispara búsquedas ni consume cuota de API.
        ...
        async function loadHomeCatalogSources() {
            // La portada nunca realiza una búsqueda nueva: reutiliza caché, reserva local y catálogo de emergencia.
            // Las APIs se consultan únicamente desde performSmartSearch después de confirmar con Enter.
        ...
        // La portada reutiliza caché/reserva local; las fuentes externas solo se consultan tras confirmar una búsqueda.
        ...
        function getFeaturedEdition(videos) {
            ...
            let previous = null;
            try { previous = JSON.parse(localStorage.getItem(FEATURED_ROTATION_STORAGE_KEY)); } catch (e) { /* estado nuevo */ }
            ...
            try { localStorage.setItem(FEATURED_ROTATION_STORAGE_KEY, JSON.stringify({ signature, offset, edition, at: Date.now() })); } catch (e) { /* almacenamiento no disponible */ }
        ...
        function initNowarfyHistory() {
            // Conservamos una entrada raíz y agregamos una entrada centinela. Así, el primer
            // Atrás de Android genera `popstate` dentro de la app en vez de sacar el documento.
        ...
        async function showSection(section, options = {}) {
            ...
            if (section === 'explore') { await renderExploreCollections(); return; }
        ...
        // Separa resultados por tipo real para no confundir una canción suelta con una lista completa.
        function renderYouTubeSearchResults(list, query) {
            ...
            // La cola principal contiene solamente videos individuales; las listas se abren antes de reproducirse.
            currentList = videos;
            const container = document.getElementById('dynamicSections');
            container.innerHTML = '';
        ...
        function buildMediaCard(song, idx) {
            ...
            div.onclick = () => {
                if (song.type === 'search_trigger' && song.query) {
                    // Al tocar un artista reciente se mantiene el filtro de canal si existe.
        ...
        function writeReserveFallback(items) {
            ...
            } catch (error) { /* almacenamiento lleno o no disponible */ }
        }
        ...
        async function reserveMarkUsed(song) {
            ...
            } catch (error) { /* la reproducción no depende de la reserva */ }
            void globalReserveRequest(GLOBAL_RESERVE_ENDPOINT, {
                method: 'POST',
        ...
        async function reserveTrim() {
            ...
            } catch (error) { /* limpieza best-effort */ }
        }
        ...
        // Elegir una canción NUEVA desde resultados/tarjetas: arranca una cola nueva de golpe,
        // reemplazando la anterior, y dispara la autogeneración (20 del mismo artista, luego similares...).
        function selectSong(song, sourceIdx) {
            ...
            // Cada elección reinicia la radio con una tanda completa detrás.
        ...
        // Agregar una canción a la cola actual sin interrumpir lo que está sonando.
        ...
        async function ensureInitialRadioQueue() {
            // Nunca reemplazar ni ampliar por red una cola restaurada al abrir la página.
            // La tanda siguiente se solicita solo después de consumir la última pista.
            if (queue.length > 0) {
            ...
            persistQueue();
            renderQueue();
            // Al abrir no se consulta la red para completar una tanda incompleta.
            // Una selección explícita de una canción sí inicia su tanda de 20.
        }
        ...
        function bindDirectAudioEvents(audio) {
            ...
            audio.onpause = () => {
                ...
                if (NOWARFY_AUDIO_PRIORITY_MODE && !playbackStoppedByUser && !audio.ended) {
                    ...
                    if (playAttempt?.catch) playAttempt.catch(() => { /* El sistema aún mantiene otro foco; el watchdog reintentará */ });
                    updateIcon();
                    updatePlaybackState();
                    return;
                }
        ...
        // Reproduce el ítem "idx" de la cola (fuente de verdad para next/prev/togglePlay).
            ...
            if (song.type === 'yt') {
                ...
                if (reuseYouTubePlayer) {
                    ...
                    try {
                        ...
                        console.debug('No se pudo reutilizar el IFrame de YouTube', error);
                        stopAll();
                ...
                if (shouldAutoplayFreeVideo) freeVideoEl.play().catch(e => { console.error(e); setTrackLoading(false); showToast('No se pudo reproducir este video libre', 'fa-triangle-exclamation'); });
                ...
                if (shouldAutoplayAudio) audioEl.play().catch(e => { console.error(e); setTrackLoading(false); showToast('No se pudo reproducir esta pista', 'fa-triangle-exclamation'); });
                else setTrackLoading(false);
            }

        }

        // --- AUTOGENERACIÓN DE LA COLA ---
        // Ronda 0: hasta 20 canciones del mismo artista (sin repetir).
        // Ronda 1+: hasta 20 canciones de artistas similares, y así sucesivamente sin fin.
        ...
        async function fetchOpenverseGenreTracks(seedSong, count) {
            ...
            } catch (e) { /* cuota o red agotada: usar caché */ }
        ...
        function isRadioMusicTrack(song) {
            ...
            // La radio automática debe ser musical: YouTube categoryId 10 es Music.
            // Las búsquedas manuales siguen pudiendo mostrar cualquier categoría.
            ...
            // Evitar clips, intros y piezas de pocos segundos que rompen la continuidad.
        ...
        async function buildAutomaticBatch(seed) {
            ...
            // Primera prioridad: canciones del mismo artista, primero frescas y luego de la reserva.
            ...
            // Si el artista no tiene suficientes resultados, completar con artistas similares.
            if (remaining() > 0) {
            ...
            // Si YouTube no alcanza, completar con música libre del mismo género en Openverse.
            if (remaining() > 0) {
                appendUniqueSongs(await fetchOpenverseGenreTracks(seed, remaining()), added, QUEUE_BATCH);
            }

            // Segunda fuente alternativa: Jamendo, consultada con el mismo estilo.
            if (remaining() > 0) {
                appendUniqueSongs(await fetchJamendoGenreTracks(seed, remaining()), added, QUEUE_BATCH);
            }
            // Último respaldo para que la radio no se quede sin contenido.
            if (remaining() > 0) {
        ...
        async function growQueueIfNeeded(force = false) {
            ...
            // Una Playlist ya armada es intocable hasta su última pista. `force` se reserva para
            // la creación inicial después de elegir una canción o para completar la radio inicial.
            ...
            } catch (e) { console.warn('No se pudo ampliar la cola automáticamente', e); }
        ...
        // --- PERSISTENCIA DE LA COLA Y DEL VIDEO ACTIVO (localStorage) ---
        function clearVideoResumeSession() {
            try { localStorage.removeItem(VIDEO_RESUME_STORAGE_KEY); } catch (e) { /* almacenamiento no disponible */ }
        ...
        function restoreQueueFromStorage() {
            try {
                ...
                if (['yt', 'mp3', 'freevideo'].includes(song.type) && resume && String(resume.resourceId) === String(song.url) && (!resume.type || resume.type === song.type) && (!resume.qid || Number(resume.qid) === Number(qid))) {
                    // Se conserva la posición para el próximo clic en reproducir, pero nunca se inicia solo.
                    pendingVideoResumeSession = resume;
                }
            } catch (e) { /* nada que restaurar */ }
        }

        // --- PANEL DE COLA: render, drag&drop, borrado ---
        ...
        function clearQueue() {
            ...
            if (!current) return;
            // "Vaciar" solo elimina lo que viene después; la canción actual sigue sonando.
            queue = [current];
            ...
            // En radio, la siguiente tanda se genera al terminar; en playlist prearmada se respeta el final de la lista.
        }
        ...
        function renderQueue() {
            ...
            queue.forEach((song, idx) => {
                ...
                row.addEventListener('dragstart', (e) => {
                    ...
                    try { e.dataTransfer.setData('text/plain', String(song._qid)); } catch (err) { /* Safari */ }
        ...
        function continueVideoAutoplay() {
            ...
            // Una cola armada siempre se consume en orden. Al llegar al final, el motor de radio
            // genera la tanda siguiente usando la pista actual como semilla; no se reemplaza por una sugerencia.
        ...
        async function recoverAutomaticQueue(targetQid) {
            ...
            try {
                ...
                // Al llegar al final, la tanda siguiente se genera usando la pista actual como semilla.
                // Esto conserva el orden completo de la Playlist actual y evita saltos prematuros.
                ...
                // Solo si no se pudo formar una tanda nueva, usar un candidato ya disponible.
                const immediateFallback = findAutomaticFallback(current);
                return appendAutomaticFallback(immediateFallback);
            } catch (error) {
                console.warn('No se pudo recuperar una pista para continuar la cola', error);
                return false;
            } finally {
                automaticQueueRecoveryInFlight = false;
            }
        ...
        function queueAutomaticAdvance(reason = 'ended') {
            ...
            const advance = async () => {
                ...
                // Primero se consume cualquier pista que ya esté en la Playlist, incluso cuando
                // la anterior terminó o falló. Solo al no existir una siguiente se genera otra tanda.
            ...
            // En segundo plano ChromeOS puede congelar los timers; una tarea de microturno
            // permite consumir una pista ya presente en la cola apenas llega ENDED. El timeout
            // breve se mantiene solo para la transición normal en primer plano.
        ...
        function writeLyricsCache(key, value) {
            try {
                ...
                // La letra sigue funcionando durante la sesión aunque el almacenamiento esté lleno.
            }
        }
        ...
        async function loadVerifiedLyrics(song) {
            ...
            try {
                ...
                // La ausencia temporal de la fuente no altera la reproducción ni muestra un botón falso.
            } finally {
        ...
        function setVideoStageMinimized(minimized, options = {}) {
            ...
            if (song.type === 'yt' && ytPlayer?.setPlaybackQuality) {
                try { ytPlayer.setPlaybackQuality(videoStageMinimized ? 'small' : 'auto'); } catch (error) { /* YouTube puede ignorar el cambio */ }
            }
        ...
        function openYouTubeCompactWindow() {
            ...
            // El navegador puede ignorar algunas opciones por seguridad; la página sigue siendo la oficial de YouTube.
            ...
            try { popup.focus(); } catch (error) { /* La ventana puede estar en otra superficie */ }
        ...
        function showVideoStage(song, options = {}) {
            ...
            if (song.type === 'yt' && ytPlayer?.setPlaybackQuality) {
                try { ytPlayer.setPlaybackQuality(hiddenDocument ? 'small' : 'auto'); } catch (error) { /* YouTube puede ignorarlo */ }
            }
        ...
        function reloadYouTubeVideoVisual(expectedUrl) {
            ...
            } catch (error) { /* El IFrame puede estar transicionando */ }
            ...
            try {
                ...
                console.debug('No se pudo solicitar el repintado oficial de YouTube', error);
                return false;
            }
        ...
        function refreshYouTubeVideoFrame(reason = 'restore') {
            ...
            const reflow = () => {
                ...
                try { ytPlayer.setSize?.(width, height); } catch (error) { /* El IFrame puede estar actualizando sus dimensiones */ }
                if (frame) {
            ...
            window.requestAnimationFrame(() => {
                ...
                videoFrameRefreshTimer = window.setTimeout(() => {
                    ...
                    window.setTimeout(() => {
                        reflow();
                        let state = null;
                        try { state = ytPlayer.getPlayerState?.(); } catch (error) { /* El IFrame puede estar transicionando */ }
        ...
        function requestYouTubePlayback(reason = 'retry') {
            ...
            try {
                ...
                console.debug('YouTube no aceptó todavía playVideo', reason, error);
                return false;
            }
        }
        ...
        function onYTReady(event) {
            ...
            // El parámetro autoplay no alcanza cuando el navegador bloquea autoplay.
            // La acción original del usuario ya habilitó reproducción: reintentamos
            // explícitamente al recibir onReady y también desde el heartbeat multimedia.
        ...
        function onYTStateChange(event) {
            ...
            if (event.data === YT.PlayerState.PLAYING) {
                if (externalAudioFocusInterrupted) {
                    try { ytPlayer?.pauseVideo?.(); } catch (error) { /* El IFrame puede estar transicionando */ }
                    return;
                }
                ...
                // Un clic en el centro del IFrame no burbujea hasta la página. Si la PWA está
                // visible y la pista ya estaba establemente sonando, este PAUSED es la señal
                // disponible de una pausa manual. Nunca lo revertimos desde el watchdog.
                ...
                // Si otra aplicación tomó el foco mientras la PWA estaba oculta, ceder durante
                // unos segundos evita que el anclaje vuelva a reclamar el audio inmediatamente.
                ...
                // ChromeOS también puede emitir PAUSED al ocultar una PWA sin que exista otra
                // aplicación de audio. En ese caso conservamos la intención y dejamos que el
                // heartbeat multimedia reintente playVideo; una pausa visible ya fue tratada arriba.
        ...
        function togglePlay() {
            ...
            // Si venimos de restaurar la cola guardada y todavía no se cargó nada en el reproductor, arrancarla.
        ...
        function nextSong(manual = false) {
            ...
            let next = idx + 1;
            // El avance automático nunca vuelve al principio de la misma tanda. El modo manual
            // puede seguir envolviendo la cola, pero el modo radio debe pedir la tanda siguiente.
            if (next >= queue.length) {
                // Tanto el avance automático como el botón Siguiente deben continuar la radio
                // desde la última pista, nunca volver al principio de la misma tanda.
        ...
        function getCurrentPlaybackVolume(song = currentQueueSong()) {
            ...
            } catch (error) { /* El reproductor puede estar cambiando de fuente */ }
        ...
        // --- MEDIA SESSION (controles del sistema / auriculares / lockscreen) ---
        function setupBackgroundPlaybackSupport() {
            // El audio HTML5 es el mecanismo compatible con la reproducción de fondo.
            // El anclaje es casi inaudible y solo busca conservar actividad multimedia; no es contenido musical.
            // No usamos un Wake Lock: impediría apagar la pantalla y gastaría batería.
            ...
            if ('mediaSession' in navigator) {
                try { navigator.mediaSession.playbackState = 'none'; } catch (e) { /* API opcional */ }
            }
        ...
        function pauseNowarfyForExternalAudio() {
            ...
            void fadeNowarfyVolume(0, 280).then(() => {
                ...
                if (song?.type === 'yt') {
                    ...
                    try { activeAudioEl?.pause?.(); } catch (error) { /* Audio ya detenido */ }
                }
            });
            persistVideoResumeSession(true);
            updateIcon();
        ...
        function setupAudioSessionSupport() {
            ...
            try { session.type = 'playback'; } catch (error) { /* API experimental o no disponible */ }
        ...
        function startSilentLoop() {
            ...
            if (playAttempt?.catch) playAttempt.catch(() => { /* Silencio: fallo no crítico */ });
        }
        ...
        function updateMediaSession(song) {
            ...
            try {
                ...
                if (!mediaSessionHandlersReady || mediaSessionSongKey !== nextSongKey) {
                    const registerAction = (action, handler) => {
                        try { navigator.mediaSession.setActionHandler(action, handler); } catch (e) { /* acción opcional */ }
                    };
            ...
            } catch (e) { /* API opcional o acción no soportada */ }
        }
        function updateMediaPosition(duration, position) {
            ...
            try { navigator.mediaSession.setPositionState({ duration, position: safePosition, playbackRate: 1 }); } catch (e) { /* API opcional */ }
        }
        function updatePlaybackState() {
            if (!('mediaSession' in navigator)) return;
            try { navigator.mediaSession.playbackState = isPlaying ? 'playing' : 'paused'; } catch (e) { /* API opcional */ }
        }
        function setupPlaybackContinuity() {
            const inspectCurrentPlayback = () => {
                ...
                if (current.type === 'mp3' && NOWARFY_AUDIO_PRIORITY_MODE && !playbackStoppedByUser && activeAudioEl?.paused && !activeAudioEl?.ended) {
                    const playAttempt = activeAudioEl.play?.();
                    if (playAttempt?.catch) playAttempt.catch(() => { /* Otra app aún tiene el foco; se revisa en el siguiente ciclo */ });
                    return;
                }
                ...
                // Nunca interrumpimos un buffer normal: llamar a playVideo() durante BUFFERING
                // puede reiniciar el segmento y se percibe como un corte periódico. La
                // continuidad solo interviene ante estados realmente detenidos.
                if (state === YT.PlayerState.BUFFERING || state === YT.PlayerState.PLAYING) return;
                // Si ChromeOS dejó el iframe pausado o sin arrancar mientras la intención
                // seguía activa, el evento timeupdate del audio auxiliar vuelve a darle play.
            ...
            // El evento multimedia del audio nativo puede seguir llegando cuando ChromeOS
            // congela los temporizadores de JavaScript. Inspecciona estado, tiempo restante
            // y reintenta playVideo solo si el usuario aún quería seguir reproduciendo.
        ...
        async function hardResetNowarfy(event) {
            ...
            try {
                // Elimina únicamente cachés de Nowarfy. No toca localStorage, IndexedDB,
                // favoritos, historial, cola ni preferencias del usuario.
                if ('caches' in window) {
                ...
                // Fallback para navegadores que no exponen Cache Storage o Service Worker.
        ...
        function setupInstallableApp() {
            ...
            window.addEventListener('load', () => {
                navigator.serviceWorker.register('/sw.js').catch(() => {
                    // La aplicación sigue operativa si el navegador no admite Service Worker.
                });
            }, { once: true });
        }
        ...
        function makeDraggable(element) {
            ...
            function dragMouseDown(e) {
                e = e || window.event;
                // No arrastrar si el clic fue sobre un control interactivo
                if (e.target.closest('button, .fav-btn, input, .volume-slider, .progress-track, .floating-restore-btn')) return;
                e.preventDefault();
                pos3 = e.clientX;
                pos4 = e.clientY;
                // Fijamos la posición actual exacta ANTES de soltar el ancla "right",
                // para que no haya un frame intermedio donde el navegador la reubique solo.
            ...
            function elementDrag(e) {
                ...
                // Mantener el reproductor dentro de la ventana visible
